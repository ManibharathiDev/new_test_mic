...
const CreateAssembly = () => {
  ...
  const addRow = () => {
    setRows([
      ...rows,
      { id: rows.length + 1, name: '', first_booth: '', last_booth: '', seq: '', status: 'Active', state_id: 1, district_id: '' }
    ]);
  };

  const handleReset = () => {
    setRows([{ id: 1, name: '', first_booth: '', last_booth: '', seq: '', status: 'Active', state_id: 1, district_id: '' }]);
  };
  ...
  const handleSubmit = (e) => {
    ...
    axios.post(URL, assemblyData, { headers }).then((response) => {
                      ...
                      <tbody>
                        {rows.map((row, index) => (
                          <tr key={row.id}>
                            <td>{row.id}</td>
                            <td>
                              <input type="text" name="name" value={row.name} onChange={(e) => handleInputChange(e, index)} />
                            </td>
                            <td>
                              <input type="text" name="first_booth" value={row.first_booth} onChange={(e) => handleInputChange(e, index)} />
                            </td>
                            <td>
                              <input type="text" name="last_booth" value={row.last_booth} onChange={(e) => handleInputChange(e, index)} />
                            </td>
                            <td>
                              <input type="text" name="seq" value={row.seq} onChange={(e) => handleInputChange(e, index)} />
                            </td>
                            <td>
